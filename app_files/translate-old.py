...
class ltr_form(FlaskForm):
	...
	ltr_lang_from = SelectField('Z języka', choices=lang_list)
	rev_lang_list = sorted(dict(lang_list).items(), key=lambda x: x[1],reverse=True)
	ltr_lang_to = SelectField('Na język', choices=rev_lang_list) 
...
def string_text(text,lang_from='pl',lang_to='uk'):
	...
	#print('\n\n******************************************\n\n')
	return [translated.text, translated.extra_data, translated.pronunciation]
